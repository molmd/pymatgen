...
class LammpsDump(MSONable):
    ...
    def as_string(self, bound_type="pp", convert=None):
        ...
        Meant to be used w/ as_txt_file method.
        :param bound_type (str): boundary type; for most cases it will be 'pp'
            for periodic boundary
        :param convert (str or None): determines which file format the dump will
            output as. Currently supports 'xyz' only. Defaults to None
            (same format as input dump file).
        :return dump_string (str): all the information in the LammpsDump object in
            string form in the format of the original dump file
        """
        if not convert:
            ...
            for line in str(self.box).split("\n"):
                bound_text = bound_text + line.split(" ")[0] + " " + \
                             line.split(" ")[1] + "\n"
            bound_text = bound_text[:-1]
            s_bounds = "ITEM: BOX BOUNDS {} {} {}\n".format(bound_type,
                                                            bound_type, bound_type) + \
                       bound_text
            data_list = [" ".join(line.split()) for line in
                         self.data.to_string(index=False).split("\n")]
            data_str = "\n".join(data_list)
    ...
    def as_txt_file(self, filename, bound_type="pp", convert=None, output=False):
        ...
        :param convert (str or None): for changing dump style or file type; see
            as_string method for details
...
def remove_unwrapped_coords(file_name, new_file_name=None):
    """
    Takes a dump file with many frames, removes the unwrapped coordinates and
    box images, and writes the new frames to a single dump file.
    :param file_name: (str) The file name of the original dump file
    :param new_file_name: (str or None) The file name of the new dump file.
        If None, the name will be the same as that of the original
    :return: None
    """
    Dump_strings = []
