...
parser.add_argument(
    "--file_pattern",
    "-fp",
    help="set file pattern before wildcard character (*), if no input, full file pattern will be *.lammpstrj",
)
parser.add_argument(
    "--timestep", "-ts", help="set # of fs per timestep, similar to lammps control file argument.", type=float
)
parser.add_argument(
    "--cutoff_time",
    "-ct",
    help="time after which data will be considered for self-diffusivity calculation. Defaults to 500 ps",
)
parser.add_argument(
    "--msd_type",
    "-mt",
    help="type of method used to calculate MSDs. Can be com (center of mass) or allatom. Defaults to com",
)
...
for i, dump in enumerate(Dumps):
    ...
    if "xu" and "yu" and "zu" not in dump.data.columns:
        ...
        assert (
            "ix" and "iy" and "iz" in dump.data.columns
        ), "Missing unwrapped coordinates and box location for converting wrapped coordinates."
    ...
    if msd_type == "allatom":
        ...
        dump.data[["dx2", "dy2", "dz2"]] = (
            dump.data[["xu", "yu", "zu"]].subtract(Dumps[0].data[["xu", "yu", "zu"]], axis=1).pow(2)
        )
        ...
        # print(msd)
        Data = Data.append(
            {"Time": time, "msd_x": msd_x, "msd_y": msd_y, "msd_z": msd_z, "msd": msd}, ignore_index=True
        )
    elif msd_type == "com":
        ...
        if i == 0:
            ...
            Initial_data = pd.concat(
                [pd.DataFrame([i + 1], columns=["Mol"]) for i in range(init_n_mols)], ignore_index=True
            )
            ...
            Initial_data[["dx2", "dy2", "dz2"]] = (
                Initial_data[["x_com", "y_com", "z_com"]]
                .subtract(Initial_data[["x_com", "y_com", "z_com"]], axis=1)
                .pow(2)
            )
        ...
        # # Creating position information of CoM of molecules for timesteps later than the first one in DataFrame then adding MSD data to Data object
            ...
            for mol in range(n_mols):
                ...
                x_com = (
                    current_dump_data["xu"].multiply(current_dump_data["mass"]).sum() / current_dump_data["mass"].sum()
                )
                y_com = (
                    current_dump_data["yu"].multiply(current_dump_data["mass"]).sum() / current_dump_data["mass"].sum()
                )
                z_com = (
                    current_dump_data["zu"].multiply(current_dump_data["mass"]).sum() / current_dump_data["mass"].sum()
                )
                Current_data.at[mol, "x_com"] = x_com
                Current_data.at[mol, "y_com"] = y_com
                Current_data.at[mol, "z_com"] = z_com

            Current_data[["dx2", "dy2", "dz2"]] = (
                Current_data[["x_com", "y_com", "z_com"]]
                .subtract(Initial_data[["x_com", "y_com", "z_com"]], axis=1)
                .pow(2)
            )
